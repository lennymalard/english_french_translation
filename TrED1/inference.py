...
model.load_state_dict(
    torch.load('parameters/parameters_02_16_2025-22_42_34.pt', weights_only=True, map_location=device))
model.eval()
...
def normalize_output(text_tokens):
    ...
    text = re.sub(r'\s+([,.!?])', r'\1', text) # Captures ws + punct and replacs with punct.
...
def translate(text, temperature=0.6, token_limit=1000):
    with torch.no_grad():
        ...
        for _ in range(token_limit):
            next_token, (h_t, c_t) = predict_next_token(decoder_inputs[:, -1:], (h_t, c_t), vocabs['fr_vocab'], temperature=temperature)
